...
class CProgramSpec extends AnyFunSuite {
  ...
  private lazy val toolchain: Option[(String, String)] = {
    ...
    prefixes.map(p => (binDir + p + "gcc", binDir + p + "objcopy")).find { case (gcc, objcopy) =>
      cmdExists(gcc) && cmdExists(objcopy)
    }
  }

  private def gcc: String = toolchain.map(_._1).getOrElse("riscv32-unknown-elf-gcc")
  private def objcopy: String = toolchain.map(_._2).getOrElse("riscv32-unknown-elf-objcopy")
  ...
  private def writeHexFromBinary(binPath: Path, hexPath: Path): Unit = {
    ...
    val padded = if (paddedLen == bytes.length) bytes else bytes ++ Array.fill[Byte](paddedLen - bytes.length)(0)
    ...
    while (i < padded.length) {
      ...
      val word = (b0 | (b1 << 8) | (b2 << 16) | (b3 << 24)).toLong & 0xffffffffL
  ...
  private def buildHexFor(cFile: Path): Path = {
    ...
    require(ccRc == 0, s"Compile failed for ${cFile.getFileName} (rc=$ccRc)")

    val objRc = Process(Seq(objcopy, "-O", "binary", elf.toString, bin.toString), repoRoot.toFile).!
    require(objRc == 0, s"objcopy failed for ${cFile.getFileName} (rc=$objRc)")
